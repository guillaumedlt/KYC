...
const KYC_STATUS_CONFIG: Record<KycStatus, { label: string; className: string }> = {
  not_started: { label: "Non démarré", className: "bg-stone-100 text-stone-500" },
  ...
  expired: { label: "Expiré", className: "bg-stone-100 text-stone-500" },
};
...
const CASE_STATUS_CONFIG: Record<CaseStatus, { label: string; className: string }> = {
  ...
  closed: { label: "Fermé", className: "bg-stone-100 text-stone-500" },
};

export function KycStatusBadge({ status }: { status: KycStatus }) {
  const c = KYC_STATUS_CONFIG[status];
  return <span className={cn("inline-block rounded-md px-1.5 py-0.5 text-[10px] font-medium", c.className)}>{c.label}</span>;
}

export function RiskBadge({ level, score }: { level: RiskLevel; score?: number | null }) {
  const c = RISK_LEVEL_CONFIG[level];
  return (
    <span className={cn("inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-[10px] font-medium", c.className)}>
