  ...
  return (
    <div>
      ...
      <div className="mb-6 flex gap-1 border-b border-border">
        {TABS.map((t) => {
          const count =
            t.key === "relations" ? relations.length :
            t.key === "screening" ? screenings.length :
            t.key === "cases" ? cases.length :
            t.key === "timeline" ? activities.length : null;
          return (
            <button
              key={t.key}
              onClick={() => setTab(t.key)}
              className={cn(
                "flex items-center gap-1.5 border-b-2 px-4 py-2.5 text-[12px] transition-colors",
                tab === t.key
            ...
            >
              ...
              {count != null && count > 0 && (
                <span className={cn(
                  "font-data text-[10px]",
                  tab === t.key ? "text-foreground/50" : "text-muted-foreground/50",
                )}>
                  {count}
                </span>
              )}
      ...
      {/* Tab content */}
      {tab === "info" && (
        <InfoTab person={person} company={company} riskFactors={riskFactors} entityType={entityType} />
      )}
      {tab === "relations" && (
        <RelationsTab relations={relations} entityId={entityId} allEntities={allEntities} />
      )}
      {tab === "screening" && <ScreeningTab screenings={screenings} />}
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-2">
      ...
      {person && (
        <div>
          ...
          <div className="space-y-3">
            ...
            <Field label="Nationalité" value={person.nationality ? `${COUNTRY_FLAGS[person.nationality] ?? ""} ${person.nationality}` : null} />
            <Field label="Résidence" value={person.country_of_residence ? `${COUNTRY_FLAGS[person.country_of_residence] ?? ""} ${person.country_of_residence}` : null} />
            <Field label="PEP" value={person.is_pep ? "Oui" : "Non"} highlight={person.is_pep} />
      ...
      {company && (
        <div>
          ...
          <div className="space-y-3">
            ...
            <Field label="Juridiction" value={company.jurisdiction ? `${COUNTRY_FLAGS[company.jurisdiction] ?? ""} ${company.jurisdiction}` : null} />
            <Field label="Forme juridique" value={company.company_type?.toUpperCase()} />
            <Field label="Secteur" value={company.industry} />
            <Field label="Immatriculation" value={company.incorporation_date ? new Date(company.incorporation_date).toLocaleDateString("fr-FR") : null} mono />
          </div>
        </div>
      )}
      ...
      {riskFactors.length > 0 && (
        <div className={person && !company ? "" : "lg:col-span-2"}>
          <span className="mb-3 block text-[11px] font-medium uppercase tracking-[0.08em] text-muted-foreground">
            Facteurs de risque ({riskFactors.reduce((sum, f) => sum + f.impact, 0)} pts)
          </span>
...
function RelationsTab({
  ...
  if (relations.length === 0) {
    return <Empty text="Aucune relation" />;
  }
...
function ScreeningTab({ screenings }: { screenings: Screening[] }) {
  if (screenings.length === 0) {
    return <Empty text="Aucun screening effectué" />;
  }
...
function CasesTab({ cases }: { cases: KycCase[] }) {
  if (cases.length === 0) {
    return <Empty text="Aucun dossier KYC" />;
  }
...
function TimelineTab({ activities }: { activities: Activity[] }) {
  if (activities.length === 0) {
    return <Empty text="Aucune activité" />;
  }
        ...
        <div key={a.id} className="flex gap-4">
          ...
          <div className="pb-6">
            ...
            <p className="mt-1.5 font-data text-[11px] text-muted-foreground/60">
              ...
              {a.agent_id && (
                <span className="ml-2 rounded bg-secondary px-1 py-px text-[9px] text-muted-foreground">IA</span>
              )}
              {a.created_by && !a.agent_id && (
                <span className="ml-2 text-muted-foreground">· Manuel</span>
              )}
            </p>
          </div>
        </div>
