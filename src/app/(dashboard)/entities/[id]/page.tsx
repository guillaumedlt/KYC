...
export default async function EntityDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const person = entity.entity_people;
  const company = entity.entity_companies;
...
function MiniStat({ label, value }: { label: string; value: string }) {
  return <div><span className="text-[9px] font-medium uppercase tracking-wider text-muted-foreground">{label}</span><p className="font-data text-[13px] font-semibold text-foreground">{value}</p></div>;
}
