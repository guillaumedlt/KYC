    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1">
          <span className="mr-2 text-[10px] text-muted-foreground">{filtered.length} entités</span>
          {FILTERS.map((f) => (
            <button key={f.key} onClick={() => setFilter(f.key)}
              className={cn("rounded px-2 py-0.5 text-[10px] transition-colors", filter === f.key ? "bg-foreground text-background" : "text-muted-foreground hover:bg-muted")}>
              {f.label}
            </button>
          ))}
        </div>
                  ...
                  <td className="px-2 py-1.5">
                    <div className="flex items-center gap-2">
                      ...
                      {entity.tags?.map((tag: string) => (
                        <span key={tag} className={cn("rounded px-1 py-px text-[9px]",
                          tag === "pep" ? "bg-amber-100 text-amber-700" : tag === "sanctions-match" ? "bg-red-100 text-red-700" : "bg-muted text-muted-foreground"
                        )}>{tag}</span>
                      ))}
                    </div>
                  </td>
                  <td className="px-2 py-1.5 text-[10px] text-muted-foreground">{TYPE_LABEL[entity.type] ?? entity.type}</td>
                  <td className="px-2 py-1.5 text-[10px]">{nat && <span>{FLAGS[nat] ?? ""} {nat}</span>}</td>
                  <td className="px-2 py-1.5">
                    {entity.risk_level ? <RiskBadge level={entity.risk_level as RiskLevel} score={entity.risk_score} /> : <span className="text-[10px] text-muted-foreground">—</span>}
                  </td>
                  <td className="px-2 py-1.5"><KycStatusBadge status={entity.kyc_status} /></td>
                  <td className="px-2 py-1.5 text-right font-data text-[10px] text-muted-foreground">
