        ...
        {(["critical", "high", "medium", "low"] as const).map((lvl) => {
          ...
          return (
            <div key={lvl}>
              ...
              <p className={cn("font-data text-[18px] font-semibold", color)}>{n}</p>
            </div>
          );
        })}
      </div>
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-1">
                    {factors.map((f, i) => (
                      <span key={i} className="text-[10px] text-muted-foreground">
                        <span className={cn("font-data font-medium", f.impact >= 20 ? "text-red-600" : f.impact >= 10 ? "text-amber-600" : "text-muted-foreground")}>+{f.impact}</span>
                        {" "}{f.factor}
                      </span>
                    ))}
                  </div>
