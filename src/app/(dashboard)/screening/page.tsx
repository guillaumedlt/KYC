            ...
            return (
              <div key={s.id} className="flex items-center justify-between rounded bg-red-50 px-3 py-1.5">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] text-muted-foreground">{TYPE_L[s.screening_type]}</span>
                  {match && <span className="text-[10px] text-red-700">{String(match.name ?? match.title ?? "")}</span>}
                </div>
            ...
            {MOCK_SCREENINGS.map((s) => {
              ...
              return (
                <tr key={s.id} className="border-b border-border/50 last:border-0 hover:bg-muted/30">
                  <td className="px-2 py-1.5"><Link href={`/entities/${s.entity_id}`} className="text-[11px] font-medium text-foreground hover:underline">{entity?.display_name}</Link></td>
