    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1">
          <span className="mr-2 text-[10px] text-muted-foreground">{filtered.length} dossiers</span>
          {FILTERS.map((f) => (
            <button key={f.key} onClick={() => setFilter(f.key)} className={cn("rounded px-2 py-0.5 text-[10px] transition-colors", filter === f.key ? "bg-foreground text-background" : "text-muted-foreground hover:bg-muted")}>{f.label}</button>
          ))}
        </div>
        <Button size="sm" className="h-6 rounded px-2 text-[10px]"><Plus className="mr-1 h-3 w-3" />Nouveau</Button>
      </div>
          ...
          <tbody>
            {filtered.map((c) => (
              <tr key={c.id} className="border-b border-border/50 last:border-0 hover:bg-muted/30">
                <td className="px-2 py-1.5"><Link href={`/cases/${c.id}`} className="font-data text-[11px] font-medium text-foreground hover:underline">{c.case_number}</Link></td>
                <td className="px-2 py-1.5">
                  {c.entities && <Link href={`/entities/${c.entity_id}`} className="text-[11px] text-foreground hover:underline">{c.entities.display_name}</Link>}
                </td>
                <td className="px-2 py-1.5 text-[11px] text-muted-foreground">{VIG[c.vigilance_level] ?? c.vigilance_level}</td>
                <td className="px-2 py-1.5 font-data text-[10px] text-muted-foreground">{c.due_date ? new Date(c.due_date).toLocaleDateString("fr-FR") : "—"}</td>
                <td className="px-2 py-1.5 text-right"><CaseStatusBadge status={c.status as CaseStatus} /></td>
              </tr>
            ))}
          </tbody>
        </table>
