...
export default function CasesPage() {
  ...
  const filtered = MOCK_CASES.filter((c) => filter === "all" || FILTERS.find((f) => f.key === filter)?.statuses.includes(c.status));
            ...
            {filtered.map((c) => {
              ...
              return (
                <tr key={c.id} className="border-b border-border/50 last:border-0 hover:bg-muted/30">
                  ...
                  <td className="px-2 py-1.5">{entity && <Link href={`/entities/${entity.id}`} className="text-[11px] text-foreground hover:underline">{entity.display_name}</Link>}</td>
