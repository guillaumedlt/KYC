        ...
        <div>
          ...
          <div className="space-y-1.5">
            {cases.filter((c: Record<string, unknown>) => !["approved", "rejected", "escalated", "closed"].includes(c.status as string)).map((c: Record<string, unknown>) => (
              <div key={c.id as string} className="flex items-center justify-between rounded-md border border-border bg-card px-4 py-2.5">
          ...
          <p className="mt-2 text-[10px] text-muted-foreground">Rapport disponible après décision</p>
        </div>
      )}
    </div>
  );
